package com.sgfm.datacenter.service.calendar.impl;
...
public class CalServiceImpl implements CalService{
	...
	public Map<String, Object> findDayPolicy(HashMap<String, Object> map) {
		Map<String, Object> dayPolicy = calDao.findDayPolicy(map);
		if (dayPolicy != null && dayPolicy.size() > 0) {
			dayPolicy.put("whetherDay", "1");
		} else {
			dayPolicy = new HashMap<String, Object>();
			dayPolicy.put("whetherDay", "0");
		}
		return dayPolicy;
	}
	
	
	//0 今天以前  1 今天以后可以约。 2今天以后可以约，但由于假日策略已经使其变为”休息“了，3 默认策略的人数导致其“已满”，4 默认策略（如下班时间到18：00后）导致其“暂停预约”，5节假日策略导致
		/*
		...
		//获取休息日的可约状态
		  public String []  getPlaydayToData(String esid,Calendar c,String[] args1,String []args2){
			  TPlaydayStrategy playdayStrate=  calDao.getPlayStgByEsid(esid);
		  ...
		  public String [] getHoildToData(String esid,Calendar calendar,String[] args1,String []args2){
			  ...
			  List<THolidayStrategy> list=  calDao.getHolidStgByEsid(map);
			  if(list==null)return args2;
			  //遍历节假日，把对应的节假日中间改为休息
		...
		public String [] getAutoToData(String esid,Calendar calendar,int thisday,String[]args2){
				 // DBRow autoStrate=  floorMerchantMgr.getAutostrategyByEsId(Long.parseLong(esid));
			TAutoStrategy ta=calDao.getAutoStgByEsidAndDate(esid);
			if(ta==null){
				ta=new TAutoStrategy();
							  ...
							  if(!hour.equals("")){
								  ...
								  try {
									Date date = sdf.parse(datestr);
									  //如果当前日期已经超过了默认时间的时分秒 ,明天是不可约的，因为calendar只是传递过来年月日，不满足比对，所以要转换为年月日时分秒
				 ...
				 if(!personCount.equals("")) {
					// DBRow[] limitPerson=  floorMerchantMgr.getPlaydayStratByEsId( Long.parseLong(esid),year+"-"+month,personCount); 
					Map<String,Object> map=new HashMap<String,Object>();
					map.put("esid", esid);map.put("creater_time", year+"-"+month);map.put("count", personCount);
					List<Map<String,Object>> res= calDao.getReservationByEcd(map);
					if(res==null) res=new ArrayList<Map<String,Object>>();
					 for(int i=0;i<res.size();i++){
						 String creater_time=(String)res.get(i).get("ORDER_TIME");// limitPerson[i].getString("creater_time");
		  ...
		  public String [] getSignToData(String esid,Calendar calendar,String[] args1,String []args2){
			  try {
				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
				  ...
				  String month=String.valueOf(calendar.get(Calendar.MONTH)+1);//得到月，因为从0开始的，所以要加1
				  month=(month.length()==1)?"0"+month:month;
				 // DBRow [] signStrate=  floorMerchantMgr.getSignStratByEsId(Long.parseLong(esid),year+"-"+month);
				  Map <String ,Object> map=new HashMap<String ,Object>();
				  map.put("esid", esid);
				  map.put("date", year+"-"+month);
				 List< TSigndayStrategy> signList=  calDao.getSignStgByEsid(map);
				  
				  
				  //遍历节假日，把对应的节假日中间改为休息 0 预约   1 休息日      2节假日
				  for(int i=0;i<signList.size();i++){
					  String type=signList.get(i).getType();
					  ...
					  if(type.equals("0")){
						...
						//  DBRow dbrow= floorMerchantMgr.getRevInfoByEsIdAndDate(Long.parseLong(esid), signStrate[i].getString("day"));
						  map.put("date", signList.get(i).getDay());
						  String count2 =String.valueOf(calDao.getRevCountByEsid(map));
