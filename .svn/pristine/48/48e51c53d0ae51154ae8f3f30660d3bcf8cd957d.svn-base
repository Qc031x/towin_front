...
$.showLoading = function(text) {
    var html ='<div class="weui-mask weui-mask--visible twui-mubu-white" style="background: rgba(244, 244, 244, 0.5);"></div>'; 
    html +=	'<div class="twui-toast twui_loading_toast twui-toast--visible" style="display:none;">';
    html += '<div class="twui_loading">';
    html += '<i class="twui-loading twui-icon_toast"></i></div>';
    html += '<p class="twui-toast_content">'+text+'</p></div>';
    $("body").append(html);
	$(".twui_loading_toast").fadeIn();
	$(".twui-mubu-white").fadeIn();
    
	interval=setInterval(loading,100);
  }
$.closeLoading = function(){
	$(".twui_loading_toast").remove();
	$(".twui-mubu-white").remove();
