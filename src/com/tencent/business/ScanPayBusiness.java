...
*/
   ...
   // private static Log log = new Log(LoggerFactory.getLogger(ScanPayBusiness.class));
    ...
    public String sendRevMsgMember(String wapurl,ScanAddOrderReqData scanPayReqData, ResultListener resultListener) throws Exception {
        ...
        //接受API返回
        String msgResponseString;

        long costTimeStart = System.currentTimeMillis();
        logger.info("sendRevMsgMember-》");

       // log.i("支付API返回的数据如下：");
        msgResponseString = scanPayService.request2(wapurl,scanPayReqData);

        long costTimeEnd = System.currentTimeMillis();
        long totalTimeCost = costTimeEnd - costTimeStart;
        return msgResponseString;
    }
